...
def prims_algorithm(G, start_vertex=1):
    n = len(G)  # Number of vertices
    visited = [False] * n  # Track visited vertices
    min_heap = []  # Min-heap to store the edges
    mst_edges = []  # To store the edges of the Minimum Spanning Tree (MST)
    
    # Start from the given start_vertex, adjust for 0-indexing
    start_vertex -= 1
    visited[start_vertex] = True

    # Add all edges from the start_vertex to the min_heap
    for neighbor, weight in G[start_vertex]:
        heapq.heappush(min_heap, (weight, start_vertex + 1, neighbor))  # (weight, u, v)

    while min_heap and len(mst_edges) < n - 1:
        # Get the smallest edge from the heap
        weight, u, v = heapq.heappop(min_heap)

        # Adjust for 1-indexing (so v should be 1-based)
        if visited[v - 1]:
            continue
...
mst = prims_algorithm(G, start_vertex=1)
